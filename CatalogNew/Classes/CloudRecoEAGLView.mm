...
- (void)renderFrameQCAR
{
    ...
    for (int i = 0; i < state.getNumTrackableResults(); ++i) {
        ...
        int currentIdFinded = [[NSString stringWithFormat: @"%s",trackable.getName()] intValue];
        if( currentIdFinded != appDelegate.currentlyTargetedMarkerId ){
            appDelegate.currentlyTargetedMarkerId = currentIdFinded;
            [viewController detectedNewMarker];
            [videoPlayerHelper[i] unload];
//            int vop = [[appDelegate.currentMarkerInfo objectForKey:@"vop"] intValue];
            int vop = 74;
            if( vop ){
//                [CatalogHelper video_filename:<#(NSString *)#> ]
//                NSString *link = [appDelegate.currentMarkerInfo objectForKey:@"link"];
//                NSArray* chunks = [link componentsSeparatedByString:@"/"];
                NSString *filename = @"74.mp4";
                NSString *documentsDirectory = [NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES) objectAtIndex:0];
                NSString *linkLocal = [documentsDirectory stringByAppendingPathComponent:filename];
                [videoPlayerHelper[i] load:linkLocal playImmediately:YES fromPosition:0.0f];
            }
//
        }
        ...
        if (0.0f == videoData[playerIndex].targetPositiveDimensions.data[0] ||
            ...
            QCAR::Vec3F vec3f = imageTarget.getSize();
//            videoData[playerIndex].targetPositiveDimensions = imageTarget.getSize();
            videoData[playerIndex].targetPositiveDimensions = QCAR::Vec2F(vec3f.data[0], vec3f.data[1]);
