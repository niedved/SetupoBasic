...
NSString * const BMPhotoAlbumLayoutAlbumTitleKindIphonePion = @"AlbumTitle";


@interface BMPhotoAlbumLayoutIphonePion ()
...
@implementation BMPhotoAlbumLayoutIphonePion
@synthesize itemSize;
...
//    self.collectionView setD
    ...
    for (NSInteger section = 0; section < sectionCount; section++) {
        ...
        for (NSInteger item = 0; item < itemCount; item++) {
            ...
            if (indexPath.item == 0) {
                UICollectionViewLayoutAttributes *titleAttributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:BMPhotoAlbumLayoutAlbumTitleKindIphonePion withIndexPath:indexPath];
    ...
    newLayoutInfo[BMPhotoAlbumLayoutAlbumTitleKindIphonePion] = titleLayoutInfo;
...
- (CGSize)collectionViewContentSize
{
    ...
    NSInteger rowCount = ceil((float)[appDelegate.bookmarksColection count] / 2.0f);
    // make sure we count another row if one is only partially filled
    if ([self.collectionView numberOfSections] % self.numberOfColumns) rowCount++;
    
    CGFloat height = self.itemInsets.top +
    (rowCount * (self.itemSize.height+40)) +
    (rowCount * self.interItemSpacingY) +
//    (rowCount * self.titleHeight) +
    self.itemInsets.bottom;
    
    NSLog(@"rowc: %d height: %f", rowCount, height);
...
{
    return self.layoutInfo[BMPhotoAlbumLayoutAlbumTitleKindIphonePion][indexPath];
}
